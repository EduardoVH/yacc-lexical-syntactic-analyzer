...
# Conjuntos de palabras reservadas y símbolos para Arduino
reserved_words = {'void', 'setup', 'loop', 'Serial', 'begin', 'print'}
symbols = {'(', ')', ';', '{', '}', '.', ');'}
identifiers = {'9600', '"Hola Mundo"', '// Por ahora en blanco'}
...
def analyze_code(code):
    ...
    for i, line in enumerate(lines, start=1):
        # Utilizar expresiones regulares para encontrar palabras y símbolos, incluyendo cadenas de texto, comentarios y el comentario especial
        words = re.findall(r'".*?"|\b\w+\b|[\(\){};]+|\/\/.*|Por ahora en blanco', line)
        for word in words:
            token = {
                'token': word,
                'line': i,
